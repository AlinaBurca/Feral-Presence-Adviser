    ...
    req.on('end', async () => {
        try {
            ...
            if (rows.length > 0) {
                ...
                sendEmail(user.email, 'Password Reset', `You requested a password reset. Click here to reset your password: ${resetUrl}`);
...
async function controllerResetPassword(req, res) {
    ...
    req.on('end', async () => {
        ...
        try {
            ...
            const [rows] = await dbConnection.promise().query('SELECT id FROM users WHERE email = ?', [email]);
            if (rows.length === 0) {
            ...
            await dbConnection.promise().query('UPDATE users SET password = ? WHERE id = ?', [hashedPassword, userId]);
            res.writeHead(200, { 'Content-Type': 'text/plain' });
