...
async function getProfileDetails(email) {
    return new Promise((resolve, reject) => {
        dbConnection.query('SELECT username, phone_number, address, email FROM users WHERE email = ?', [email], (error, results) => {
            if (error) {
                return reject(error);
            }
            if (results.length > 0) {
...
async function updateProfileDetails(email, fullName, contactNumber, address) {
    return new Promise((resolve, reject) => {
        dbConnection.query('UPDATE users SET username = ?, phone_number = ?, address = ? WHERE email = ?', [fullName, contactNumber, address, email], (error, results) => {
            if (error) {
                return reject(error);
            }
            resolve(results);
...
async function handleProfileUpdate(body, res) {
    ...
    try {
        const profile = await updateProfileDetails(body.email, fullName, contactNumber, address);
        console.log(profile);
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: true, message: 'Profile updated succesfully' }));
        return;
    } catch (error) {
...
async function getPassword(email) {
    console.log(email);
    return new Promise((resolve, reject) => {
        dbConnection.query('SELECT password FROM users WHERE email = ?', [email], (error, results) => {
            if (error) {
                return reject(error);
            }
            if (results.length > 0) {
...
async function updatePassword(email, new_password) {
    return new Promise((resolve, reject) => {
        dbConnection.query('UPDATE users SET password = ? WHERE email = ?', [new_password, email], (error, results) => {
            if (error) {
                return reject(error);
            }
            resolve(results);
...
async function handlePasswordUpdate(body, res) {
    ...
    console.log(JSON.stringify(actual_password.password));
    ...
    try {
        if (!isMatch) {
            ...
            res.end(JSON.stringify({ success: true, message: 'Password deosn t match' }));
            return;
        }
        else {
            const profile = await updatePassword(JSON.parse(body.sessionId).email, newHashedPassword);
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ success: true, message: 'Profile updated succesfully' }));
            return;
        }
